...
function func(bitstring, min_grid, max_grid)

    """
    Convert a binary vector (bitstring) to a grid value and then evaluate a given function with that input value

    Inputs:
    bitstring = binary vector of 1s and 2s e.g. [1, 2, 2, 1]
    ...
    delta = (max_grid - min_grid)/(2^N-1)

    x = min_grid + foldl((acc, b) -> (acc << 1) | b, bitstring.-1)*delta
...
function get_decomposition_and_updated_pivots(l, row_pivots, col_pivots, min_grid, max_grid, max_pivots, tolerance, func, func_cache)
    ...
    all_rows = []
    all_cols = []
    Pi = zeros(ComplexF64, length(row_pivots_l), 2, 2, length(col_pivots_l))
    flag = true
    for row_s_idx in [1, 2] # The order of the for loops for the rows here is important to match the basis order stored in all_rows, all_cols with the reshape of the Pi tensor done below
        for (row_idx, row) in enumerate(row_pivots_l)
            ...
            for (col_idx, col) in enumerate(col_pivots_l) # The order of the for loops for the columns here is important to match the basis order stored in all_rows, all_cols with the reshape of the Pi tensor done below
                for col_s_idx in [1, 2]
                    ...
                    bitstring = vcat(row, row_s_idx, col_s_idx, col)
                    if haskey(func_cache, bitstring) # If the function has already been evaluated at a given bitstring we get the value from a dictionary cache
                        x, y = func_cache[bitstring]
                    else
                        x, y = func(bitstring, min_grid, max_grid)
                        func_cache[bitstring] = (x, y)
                    end
    ...
    Pi = reshape(Pi, length(row_pivots_l)*2, 2*length(col_pivots_l))
...
function tci(N, func, min_grid, max_grid, tolerance, max_pivots, sweeps)
    ...
    for i in 1:N
        if i == 1
            push!(mps_list, zeros(ComplexF64, 2, 1))
        elseif i == N
            push!(mps_list, zeros(ComplexF64, 1, 2))
        else
            push!(mps_list, zeros(ComplexF64, 1, 2, 1))
        end
    end
    bonddims = [1 for _ in 1:N-1]
    bonddims_previous = copy(bonddims)

    # Initialize the pivots with a random bitstring
    initial_bitstring = [rand([1, 2]) for _ in 1:N]
    row_pivots = [[initial_bitstring[1:l]] for l in 0:N-1]
    col_pivots = [[initial_bitstring[l+1:N]] for l in 1:N]
    ...
    for sweep in 1:sweeps
        ...
        for dir in (true, false)  # true = forward, false = backward
            ...
            for l in range_l
                ...
                P, L, L11, L21, D, U, U11, U12, Q, new_row_pivots, new_col_pivots, pivot_error, func_cache = get_decomposition_and_updated_pivots(l, row_pivots, col_pivots, min_grid, max_grid, max_pivots, tolerance, func, func_cache)
                ...
                if l == 1
                    left_tensor = reshape(left_tensor, 2, size(left_tensor, 2)) # Open boundary conditions have no left index for the first MPS tensor
                else
                    left_tensor = reshape(left_tensor, div(size(left_tensor, 1), 2), 2, size(left_tensor, 2)) # Indexing is: left bond index, physical index, right bond index
                end
                if l == N-1
                    right_tensor = reshape(right_tensor, size(right_tensor, 1), 2) # Open boundary conditions have no right index for the last MPS tensor
                else
                    right_tensor = reshape(right_tensor, size(right_tensor, 1), 2, div(size(right_tensor, 2), 2))
                end
...
max_pivots = 256
sweeps = 200
mps_list, pivot_error_list, func_cache, sweep, bonddims = tci(N, func, min_grid, max_grid, tolerance, max_pivots, sweeps)
...
mps_vec = []
for i in 0:2^N-1
    binary_list = decimal_to_padded_binary_list(i, N) 
    push!(mps_vec, contracted_mps[binary_list...])
end
p = plot(title = "N = $(N), Sweeps = $(sweep), Tolerance = $(tolerance), Max Pivots = $(max_pivots)\nBond dimensions: $(bonddims)")
delta = (max_grid-min_grid)/(2^N-1)
x_values = [min_grid + delta*i for i in 0:2^N-1]
y_values = [func(decimal_to_padded_binary_list(i, N), min_grid, max_grid)[2] for i in 0:2^N-1] 
scatter!(x_val, y_val, label = "Function evaluations, $( @sprintf("%.1f", length(x_val) * 100 / 2^N) )% of domain", m = (2, :white, stroke(1, :black)))

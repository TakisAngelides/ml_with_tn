...
h = [1 1; 1 -1] # hadamard
ITensors.op(::OpName"h", ::SiteType"S=1/2") = h # hadamard as ITensor
...
function mps_list_to_itensors_mpo(N, mps_list, sites, d)
    ...
    links = [Index(size(mpo_list[n], 1), "Link,i=$n") for n in 2:N]
...
max_pivots = 256
sweeps = 100
num_starting_pivots = 40
sites = siteinds(d, N)
qft_sign_convention = -1
qft_mpo_cutoff = 1e-16
cutoff_val = 1e-16
kx_positive = LinRange(0, round(n/2), Int(n/2)+1)*2*pi/(max_grid - min_grid) # grid of frequencies
kx_negative = -kx_positive[2:(end-1)] # one less freq negative than positive
kx = [kx_positive; reverse(kx_negative)] # as they appear after the fft

# Initializing pivots and mps list
row_pivots, col_pivots, func_cache = initialize_pivots_and_cache(func_mps, min_grid, max_grid, d, N, num_starting_pivots)
mps_list, row_pivots, col_pivots = get_mps_from_pivots(row_pivots, col_pivots, min_grid, max_grid, d, N, func_cache, max_pivots, tolerance, func_mps)

# Perform TCI
mps_list, pivot_error_list, func_cache, sweep, bonddims = tci(N, func_mps, min_grid, max_grid, tolerance, max_pivots, sweeps, d, func_cache, row_pivots, col_pivots, mps_list)
mps = mps_list_to_itensors_mps(N, mps_list, sites)
...
scatter!(p, x_values, real(mps_to_plot))
display(p)

# Perform QFT and plot with circuit method
qft_mpo = sqrt(1/n)*get_qft_mpo(sites, qft_sign_convention, qft_mpo_cutoff)
println(linkdims(qft_mpo))
mps_qft = reverse_mps(apply(qft_mpo, mps; cutoff = cutoff_val), sites)
mps_qft_to_plot = mps_to_list(mps_qft)
p = plot()
scatter!(p, kx, abs.(mps_qft_to_plot))
display(p)

# Perform QFT and plot with TCI method
d = 4
row_pivots, col_pivots, func_cache = initialize_pivots_and_cache(func_qft, min_grid, max_grid, d, N, num_starting_pivots)
mps_list, row_pivots, col_pivots = get_mps_from_pivots(row_pivots, col_pivots, min_grid, max_grid, d, N, func_cache, max_pivots, tolerance, func_qft)
mps_list, pivot_error_list, func_cache, sweep, bonddims = tci(N, func_qft, min_grid, max_grid, tolerance, max_pivots, sweeps, d, func_cache, row_pivots, col_pivots, mps_list)
qft_tci_mpo = mps_list_to_itensors_mpo(N, mps_list, sites, d)
mps_qft_tci = reverse_mps(apply(qft_tci_mpo, mps; cutoff = cutoff_val), sites)
mps_qft_tci_to_plot = mps_to_list(mps_qft_tci)
p = plot()
scatter!(p, kx, abs.(mps_qft_tci_to_plot))
display(p)

println(norm(qft_mpo - qft_tci_mpo))
